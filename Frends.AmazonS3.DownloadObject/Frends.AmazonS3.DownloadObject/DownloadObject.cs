...
namespace Frends.AmazonS3.DownloadObject
{
    ...
    public class AmazonS3
    {
        ...
        /// </summary>
        /// <param name="input">Connection and file parameters</param>
        ...
        private static async Task<List<SingleResultObject>> DownloadUtility(Input input, CancellationToken cancellationToken, bool preSigned)
        {
            ...
            if (preSigned)
            {
                ...
                results.Add(new SingleResultObject(await WriteToFilePreSigned(input.DestinationPath, input.FileName, responseStream, input)));
            }
            else
            {
                ...
                using (client)
                {
                    ...
                    foreach (var fileObject in allObjectsInDirectory)
                    {
                        ...
                        if (mask.IsMatch(fileObject.Key.Split('/').Last()) && (targetPath.Split('/').Length == fileObject.Key.Split('/').Length || !input.DownloadFromCurrentDirectoryOnly) && !fileObject.Key.EndsWith("/") && fileObject.Key.StartsWith(input.S3Directory))
                        {
                            ...
                            if (File.Exists(fullPath) && !input.Overwrite && !input.ContinueIfExists)
                                throw new Exception($"File {fileObject.Key.Split('/').Last()} already exists at {fullPath}. Consider using Overwrite or ContinueIfExists options.");
                            else if (File.Exists(fullPath) && !input.Overwrite && input.ContinueIfExists)
                                results.Add(new SingleResultObject($"File {fileObject.Key.Split('/').Last()} was skipped because it already exists at {fullPath} and ContinueIfExists is true. Set Overwrite to true to overwrite the file."));
                            else
                            {
        ...
        private static async Task<string> WriteToFilePreSigned(string destinationPath, string fileName, Stream responseStream, Input input)
        {
            string responseBody;
            ...
            {
                throw new Exception($"File already exists at {fullPath}. Set Overwrite to true to overwrite the file.");
            }
            if (File.Exists(fullPath) && input.Overwrite)
            {
        ...
        private static async Task<string> WriteToFile(Input input, S3Object fileObject, AmazonS3Client s3Client, string destinationFolder, string fullPath)
        {
            string responseBody;
