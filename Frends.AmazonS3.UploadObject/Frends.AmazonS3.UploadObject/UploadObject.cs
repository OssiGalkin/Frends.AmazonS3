...
public class AmazonS3
{
    ...
    /// <returns>List { string UploadedObject }</returns>
    public static async Task<Result> UploadObject([PropertyTab] Connection connection, [PropertyTab] Input input, CancellationToken cancellationToken)
    {
        ...
        if (connection.ThrowErrorIfNoMatch && filesToCopy.Length < 1) throw new Exception($"No files match the filemask within supplied path. {nameof(input.FileMask)}");

        var results = await ExecuteUpload(filesToCopy, input, connection, cancellationToken);

        return new Result(results);
    }

    private static async Task<List<UploadResult>> ExecuteUpload(IEnumerable<FileInfo> filesToCopy, Input input, Connection connection, CancellationToken cancellationToken)
    {
        ...
        foreach (var file in filesToCopy)
        {
            ...
            if (file.FullName.Split(Path.DirectorySeparatorChar).Length > input.FilePath.Split(Path.DirectorySeparatorChar).Length && connection.PreserveFolderStructure)
            {
                ...
                result.Add(new UploadResult(connection.ReturnListOfObjectKeys ? $@"Upload complete: {fullPath}" : $@"Upload complete: { file.FullName }"));
            }
            else
            {
                switch (connection.AuthenticationMethod)
                ...
                result.Add(new UploadResult(connection.ReturnListOfObjectKeys ? $@"Upload complete: {input.S3Directory + file.Name}" : $@"Upload complete: { file.FullName }"));
            }
